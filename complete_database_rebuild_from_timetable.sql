-- COMPLETE DATABASE REBUILD FROM ACTUAL TIMETABLE
-- This script completely rebuilds all tables to match the exact timetable provided
...
-- Note: Sequences will auto-increment, no need to reset them

-- Insert all sites from the timetable
...
-- Insert all 75 employees with exact data from timetable
...
-- Insert rest periods from timetable (exact dates from the image)
...
-- Insert annual leave periods from timetable (exact dates from the image)
...
-- Insert site assignments from timetable
INSERT INTO shift_assignments (id, employee_id, site_id, assigned_date, shift_type, notes, created_at, updated_at)
SELECT 
    gen_random_uuid(),
    e.id,
    s.id,
    '2025-10-22'::date,
    'day',
    'Assigned from timetable',
    NOW(),
    NOW()
FROM employees e
CROSS JOIN sites s
WHERE s.name IN ('Peridon', 'Bedford', 'Beukeskuil', 'Aliwal Noord', 'BA Treasury', 'Buanodonna', 'Down Touch', 'Harmony Piggeries', 'Hartzview', 'EP Hills', 'MJ Honiball', 'CF Haasbroek', 'UTG Boerdery', 'Khamab', 'Mulda Boerdery', 'Arthur Nel', 'SAC Trucks', 'AF van Wyk', 'WF van der Rust');
